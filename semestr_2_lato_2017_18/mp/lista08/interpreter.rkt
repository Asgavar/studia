...
(define (append-eval e)
  (append (second e) (third e)))
...
(define (map-eval e env)
  (map (lambda (x) (apply-closure (eval-env (second e)
                                            env)
                                  (list x)))
       (third e)))
...
(define (list-selector-eval e)
  (let ([chars (split-symbol (car e))])
    ((apply compose
           (filter (lambda (x) (not (void? x)))
                   (map (lambda (c) (cond [(eq? c 'a) car]
                                          [(eq? c 'd) cdr]))
                        chars))) (second e))))
...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(map? e) (map-eval e env)]
        [(reverse? e) (reverse-eval e)]
        [(list-selector? e) (list-selector-eval e)]
        [(var? e)
