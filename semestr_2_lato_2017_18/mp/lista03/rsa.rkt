...
(define (solve-ax+by=1 a b)
  (if (= a 0)
      (cons 0 1)
      (let* ([bezout-coefficients (solve-ax+by=1 (modulo b a) a)]
             [x (car bezout-coefficients)]
             [y (cdr bezout-coefficients)])
        (cons (- y (* (quotient b a) x)) x))))
...
(define (RSA-unconvert-list intlist key)
  (let ([n (key-modulus key)])
    (define (unconvert l prev-encoded)
      (if (null? l)
          '()
          (let* ([x-with-prev (RSA-transform (car l) key)]
                [x (modulo (+ x-with-prev prev-encoded) n)])
...
;;; Message and its signature - just a plain old pair
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let* ([encrypted-msg (RSA-encrypt message recipient-public-key)]
         [encrypted-hash (compress encrypted-msg)]
         [priv-key-exponent (key-exponent sender-private-key)]
         ;; since the hash is not a string we must use the low-level
         ;; RSA-transform procedure
         [signature (RSA-transform encrypted-hash sender-private-key)])
    (make-signed-message encrypted-msg signature)))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let* ([recv-message (message cyphertext)]
         [recv-signature (signature cyphertext)]
         [recv-hash (RSA-transform recv-signature sender-public-key)]
         [msg-hash (compress recv-message)])
    (if (not (= msg-hash recv-hash))
        (raise "Message was forged!")
        (RSA-decrypt recv-message recipient-private-key))))
...
(define (execute-tests)
  ...
  (newline)
  (display
   (solve-ax+by=1 4398051753985 9223376434904432641))

  (newline)

  (define encrypted1
    (RSA-encrypt "abeeeeffff" (key-pair-public test-key-pair1)))

  (newline)
  
  (display
   (RSA-decrypt encrypted1 (key-pair-private test-key-pair1)))
  ...
  (display
   (RSA-decrypt
    (RSA-encrypt
     "9/11 was an inside job" (key-pair-public test-key-pair2))
    (key-pair-private test-key-pair2)))

  (newline)
  
  (display
   (let ([public1 (key-pair-public test-key-pair1)]
         [private1 (key-pair-private test-key-pair1)]
         [public2 (key-pair-public test-key-pair2)]
         [private2 (key-pair-private test-key-pair2)])
     (define signed-and-encrypted
       (encrypt-and-sign "jet fuel can't melt steel beams" public2 private1))
     (authenticate-and-decrypt signed-and-encrypted private2 public1))))
